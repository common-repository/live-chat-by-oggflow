...
load_plugin_textdomain('wp-content/plugins/live-chat-by-oggflow');
...
function OGGFLOW_dashboard() {
        global $plugurldir; 
    echo '<div id="dashboarddiv"><div><img src="'.$plugurldir.'/oggflow.png"/><br/></div> <div><a href="https://oggflow.icoa.com/#Dashboard" class="button-primary" target="_newWindow" >Dashboard</a></div> <br/> <div><a href="https://oggflow.icoa.com/#Conversations" class="button-primary" target="_newWindow" >Conversations</a></div> <br/> <div><a href="https://oggflow.icoa.com/#LiveAgent" class="button-primary" target="_newWindow" >Live Chats</a></div> <br/> <div><a href="https://oggflow.icoa.com/#Website-'.get_option('oggflowID').'" class="button-primary" target="_newWindow" >Web Widget Settings</a></div> </div> ';
}
...
function oggflow_plugin_actions($links, $file) {
    ...
    if($file == $this_plugin && function_exists('admin_url')) {
        $settings_link = '<a href="'.admin_url('options-general.php?page=live-chat-by-oggflow').'">'.__('Settings', $_SERVER['SERVER_NAME']).'</a>';
...
<div class="wrap">
        <?php screen_icon() ?>
    <h2><?php _e('Live Chat by OggFlow', $_SERVER['SERVER_NAME']) ?></h2>
    <div class="metabox-holder meta-box-sortables ui-sortable pointer">
        <div class="postbox" style="float:left;width:30em;margin-right:10px">
			...
			<input type="text" name="oggflowID" id="oggflowID" placeholder="Your Chat Widget Key" value="<?php echo(get_option('oggflowID')) ?>" style="width:100%" />
                    <p class="submit" style="padding:0"><input type="hidden" name="action" value="update" />
                        ...
                        <input type="button" name="oggflowGenerate" id="oggflowGenerate" value="<?php _e('Generate Key', $_SERVER['SERVER_NAME']) ?>" class="button-primary" style="margin:3px;"/> 
			<input type="submit" name="oggflowSubmit" id="oggflowSubmit" value="<?php _e('Save Settings', $_SERVER['SERVER_NAME']) ?>" class="button-primary" /> 
			</p>
                 <small class="nonessential"><?php _e('<br>Your Chat Widget Key is located at OggFlow.com under Live Chat --> Chat Widgets --> Your Domain.', $_SERVER['SERVER_NAME']) ?></small></p>
                   </form>
            </div>
        </div>
        <div class="postbox" style="float:left;width:38em">
            <h3 class="hndle"><span id="noAccountSpan"><?php _e('No Account?  Sign up now for free!', $_SERVER['SERVER_NAME']) ?></span></h3>
		...
		<p><?php printf(__('OggFlow is a powerful live chat and customer service suite that includes email ticketing, social media feeds, 
			...
			Please visit %1$sLive Chat and Live Help Software for your website%2$sOggFlow.com%3$s to 
					learn more.', $_SERVER['SERVER_NAME']), '<a href="http://www.oggflow.com/" title="', '">', '</a>') ?></p>
...
$(document).on("click", "#oggflowRegister", function () {
 ...
 $.ajax({
   type: 'GET',
    ...
    <?php }
    add_submenu_page('options-general.php', __('OggFlow Settings', $_SERVER['SERVER_NAME']), __('OggFlow Settings', $_SERVER['SERVER_NAME']), 'manage_options', 'live-chat-by-oggflow', 'oggflow_settings_page');
}
